...
variable "volume_size" {
  description = "Taille du disque root en GB"
  type        = number
  default     = 50
}
...
# ============================================================
locals {
  node_roles = ["master", "worker-1", "worker-2"]
  private_ips = [
    "192.168.2.10",
    "192.168.2.11",
    "192.168.2.12",
...
# ============================================================
# Instances EC2 (Master + 2 Workers)
# ============================================================
resource "aws_instance" "nodes" {
  ...
  tags = {
    Name    = "${var.project_name}-${local.node_roles[count.index]}"
...
resource "aws_eip" "nodes" {
  ...
  tags = {
    Name    = "${var.project_name}-eip-${local.node_roles[count.index]}"
    Project = var.project_name
  }
}
...
output "nodes" {
  ...
  value = {
    for i, role in local.node_roles : role => {
      private_ip = aws_instance.nodes[i].private_ip
      public_ip  = aws_eip.nodes[i].public_ip
    }
  }
}
...
output "cluster_summary" {
  ...
  value = {
    master  = aws_eip.nodes[0].public_ip
    workers = [aws_eip.nodes[1].public_ip, aws_eip.nodes[2].public_ip]
    region  = var.region
  }
}
